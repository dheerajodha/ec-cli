...
func ValidateImageWithVSACheck(ctx context.Context, comp app.SnapshotComponent, snap *app.SnapshotSpec, p policy.Policy, evaluators []evaluator.Evaluator, detailed bool, vsaExpiration time.Duration, rekorURL string) (*output.Output, error) {
	...
	if vsaExpiration > 0 && rekorURL != "" {
		...
		if err != nil {
			...
			// Create a minimal output indicating validation was skipped
			out := &output.Output{
			...
			// Mark all checks as passed since we have a valid VSA
			out.SetImageAccessibleCheckFromError(nil)
			out.SetImageSignatureCheckFromError(nil)
			out.SetAttestationSignatureCheckFromError(nil)
			out.SetAttestationSyntaxCheckFromError(nil)
			out.SetPolicyCheck([]evaluator.Outcome{}) // No violations
